...
var KTDatatablesServerSide = function () {
    ...
    var handleSearchDatatable = function () {
        ...
        const filterNumberSearch = document.querySelector('[data-kt-docs-table-filter="code"]');
        ...
        $(document).on('keyup', 'input[data-kt-docs-table-filter="code"]', function (e) {
            // dt.search(e.target.value).draw();
            dt.search(e.target.value, 'account_number', true, false).clear().draw();
        });
        ...
        $(document).on('change', '#change_order_filter_status', e => {
            let $this = $(e.currentTarget);
            myData.order_status = $this.val();
            dt.ajax.reload();
        });

        $(document).on('change', '#change_account_level_filter', e => {
            let $this = $(e.currentTarget);
            myData.account_level = $this.val();
            $('#oc_datatable').attr('data-level', $this.val());
            $('#oc_datatable').removeClass(`oc_datatable1`);
            $('#oc_datatable').addClass(`oc_datatable${$this.val()}`);
            // $('.check-all').removeClass(`check-all1${$this.val()}`);
            // $('.check-all').removeClass(`check-all1`);
            var elements = document.querySelectorAll('.check-all');
            ...
            $('.sorting_asc').html(`<div class="form-check form-check-sm form-check-custom form-check-solid me-3">
                                            <input class="form-check-input check-all check-all${$this.val()}" type="checkbox" value="${$this.val()}">
                                        </div>`);
            $('.check-all').addClass(`check-all${$this.val()}`);
            ids = [];
            dt.ajax.reload();
        });
    ...
    var handleSubmitForm = () => {
        ...
        // Get elements
        const form = document.getElementById('add_form_modal');
        const formjs = $("#add_form_modal");
        const submitButton = document.getElementById('form_btn_submit');
    ...
    var VertifyReqOperation = () => {
        $(document).on('click', '#oc_datatable .vertify', e => {
            e.preventDefault();
            let $this = $(e.currentTarget);
            let target_url = $this.data('url');
            let title = $this.data('title');
    ...
    var handleExport = () => {
        ...
        $(document).on('click', '#exportExcel', function (e) {
            ...
            let exportExcelButton = $(this);
            exportExcelButton.attr('data-kt-indicator', 'on');
            exportExcelButton.attr('disabled', true);
            axios.get(`${url}`).then(response => {
                if (response.data.status == true) {
                    exportExcelButton.attr('data-kt-indicator', 'off');
